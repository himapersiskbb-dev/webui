...
export default {
  ...
  setup() {
    ...
    const filteredMode = ref(null);
    const latest = ref(true);
    const store = useStore();

    const articlesToRender = computed(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const response = await Promise.all([
          store.dispatch("articles/loadArticles", { latest: latest.value }),
          store.dispatch("articles/loadCategories"),
        ]);
        articles.value = response[0].data;
